    ...
    async def _franchise_delete(
        ...
        if fdata.teams:
            for t in fdata.teams:
                tier = t.tier
                if not tier:
                    log.error(f"Team {t.name} in franchise {fdata.name} has no tier assigned. Skipping free agency role assignment.")
                    await interaction.followup.send(
                        content=(f"Team {t.name} in franchise {fdata.name} has no tier assigned. Must process player roles manually."),
                        ephemeral=True,
                    )
                    continue
